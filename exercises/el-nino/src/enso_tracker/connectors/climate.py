...
class CPCOniConnector(Connector):
    ...
    The CPC page is an HTML table of ``SEAS YR TOTAL ANOM`` rows. It has been
    stable for years but we parse defensively: any row that does not yield a
    known season plus two floats is dropped and counted, and the count is
    surfaced as a QC statistic rather than swallowed.
    """

    dataset = "oni_observed"

    def fetch(self) -> FetchResult:
        html = self.http_get(self.url)
        ...
        for raw in re.findall(r"<tr[^>]*>(.*?)</tr>", html, flags=re.S | re.I):
            ...
            if len(cells) < 4 or cells[0].upper() not in SEASONS:
                dropped += 1
                continue
            try:
                rows.append({
                    "season": cells[0].upper(),
                    "year": int(cells[1]),
                    "sst": float(cells[2]),
                    "oni": float(cells[3]),
                })
        ...
        if not rows:
            raise SourceUnavailable(
                f"{self.name}: parsed 0 usable rows -- the CPC table layout has "
                f"probably changed. Refusing to return an empty frame."
            )
        ...
        return FetchResult(
            ...
            source_url=self.url,
            valid_time=valid,
            notes=[f"dropped {dropped} non-data rows"] if dropped else [],
        )
