    ...
    def fetch_authenticated(self) -> FetchResult:
        try:
            ...
            raise SourceUnavailable(
                f"{self.name}: install the geo extra -- pip install '.[geo]'"
            ) from exc
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                raise SourceUnavailable(f"{self.name}: CDS retrieve failed ({exc})") from exc
        ...
        df["source_system"] = request["originating_centre"]

        return FetchResult(
            dataset=self.dataset,
            frame=df,
            source=f"Copernicus C3S ({request['originating_centre']})",
            source_url=self.url,
        )
