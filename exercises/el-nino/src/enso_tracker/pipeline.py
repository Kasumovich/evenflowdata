...
def step_a_ingest(
    ...
    *,
    ...
    def record_failure(name: str, note: str) -> None:
        ...
        # Say it out loud. These notes used to travel only in the payload, so a
        # source that attempted and failed produced no log line whatsoever --
        # identical, from the operator's seat, to a source that was never
        # configured. Three Copernicus feeds failed with real API errors and the
        # log showed nothing at all, which cost a debugging round.
        log.warning("%s", note)
        if name in history:
    ...
    for name, spec in blocks.items():
        ...
        if spec.get("auth") == "key" and not credentials_present(spec):
            ...
            note = (
                f"{name}: dormant (credentials absent"
                + (f"; missing {', '.join(missing)}" if missing else "")
                + ")"
            )
            metrics.notes.append(note)
            # Naming *which* variable is missing is the difference between one
            # more run and none: `CDSAPI_URL` unset while `CDSAPI_KEY` was set
            # looked exactly like no credentials at all.
            log.info("%s", note)
            continue
