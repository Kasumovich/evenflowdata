...
def test_oni_age_limits_sit_above_the_healthy_peak(cfg):
    """The ONI's age sawtooths between ~50 and ~89 days by construction.

    A season is a 3-month mean stamped at its centre month and cannot be
    published until the season closes, so the day before a release the newest
    value is 89 days old with nothing wrong. Both thresholds must therefore sit
    ABOVE that, or the alarm fires on healthy data every single month -- and an
    alarm that cries wolf is how the six-year-old Nino 3.4 value survived.
    """
    HEALTHY_PEAK_DAYS = 89
    for dataset in ("oni_observed", "nino34_monthly"):
        spec = cfg.thresholds["alerts"]["observation_age_h"][dataset]
        assert spec["warn"] / 24 > HEALTHY_PEAK_DAYS, dataset
        assert spec["critical"] / 24 > spec["warn"] / 24, dataset
        # and still tight enough to catch a genuinely missed release cycle
        assert spec["critical"] / 24 <= 130, dataset


def test_weekly_limits_stay_tighter_than_the_monthly_ones(cfg):
    """The whole point of per-dataset limits: a weekly product that is 30 days
    old is broken, while a monthly one that is 30 days old is early."""
    limits = cfg.thresholds["alerts"]["observation_age_h"]
    assert limits["weekly_nino"]["critical"] < limits["oni_observed"]["warn"]
